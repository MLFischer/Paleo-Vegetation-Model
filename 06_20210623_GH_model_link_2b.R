...
get(load("20201125_AHP01_CAB_LC_ET.RData"))
...
d2a$pre    <- round(d2a$pre,0)
nr <- which(d2a$pre == comparison)
...
par(mfrow = c(1,3))
plot(PRE,alti, type="p",ylim=c(490,550),xlab="Precipiation",ylab="Transitiontime",xlim=c(1,1.5))
##
plot(PRE,dva,type="p",col="slateblue2",lty=3, ylim=c(0,10),xlim=c(1,1.5))
points(PRE,dvb,col="slateblue2",lty=2,pch=16)
points(PRE,dvc,col="black")


rel2<- dvb/(dvb_vol_p+dvb)
plot(PRE,rel2,type="p",xlim=c(1,1.5))#,ylim=c(0.1,0.3))


alti[which(alti[-1]-alti[-50]!=0)][-1]
PRE[which(alti[-1]-alti[-50]!=0)][-1]
